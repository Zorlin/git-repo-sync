...
function sync_pass(){
    ...
    if [[ $env_allow_async == 1 && -n "$fetch_spec_a" && -n "$fetch_spec_b" ]]; then
        ...
        git fetch --tags $origin_a $fetch_spec_a > "$path_async_output/fetch_a.txt" &
        pid_fetch_a=$!
        git fetch --tags $origin_b $fetch_spec_b > "$path_async_output/fetch_b.txt" &
        pid_fetch_b=$!
        ...
        if [[ -n "$fetch_spec_a" ]]; then
            ...
            echo $fetch_spec_a
            git fetch --tags $origin_a $fetch_spec_a
        fi;
        if [[ -n "$fetch_spec_b" ]]; then
            echo;echo "> Fetch $origin_b (sync)"
            echo $fetch_spec_b
            git fetch --tags $origin_b $fetch_spec_b
        fi;
    fi;
    ...
    if [[ $env_allow_async == 1 && -n "$post_fetch_spec_a" && -n "$post_fetch_spec_b" ]]; then
        ...
        git fetch --tags $origin_a $post_fetch_spec_a > "$path_async_output/post_fetch_a.txt" &
        pid_post_fetch_a=$!
        git fetch --tags $origin_b $post_fetch_spec_b > "$path_async_output/post_fetch_b.txt" &
        pid_post_fetch_b=$!
        ...
        if [[ -n "$post_fetch_spec_a" ]]; then
            ...
            echo $post_fetch_spec_a
            git fetch --tags $origin_a $post_fetch_spec_a
        fi;
        if [[ -n "$post_fetch_spec_b" ]]; then
            echo;echo "> Post-fetch $origin_b (sync)"
            echo $post_fetch_spec_b
            git fetch --tags $origin_b $post_fetch_spec_b
        fi;
    fi;
}
sync_pass
